...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (registrationForm) {
        registrationForm.addEventListener('submit', async (e) => {
            ...
            try {
                const response = await fetch('http://localhost:4200/api/student/register', {
                    method: 'POST',
                    headers: {
    ...
    if (statusForm) {
        statusForm.addEventListener('submit', async (e) => {
            ...
            try {
                const response = await fetch(`http://localhost:4200/api/student/status/${studentId}`, {
                    method: 'GET'
                });
    ...
    if (loginForm) {
        loginForm.addEventListener('submit', async (e) => {
            ...
            try {
                const response = await fetch('http://localhost:4200/api/student/login', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json'
                    },
